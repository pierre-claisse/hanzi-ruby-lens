...
- [ ] T001 [P] [US1] Create usePinyinVisibility hook test file in src/hooks/usePinyinVisibility.test.ts - Test: Returns [true, function] on first run (no saved preference)
- [ ] T002 [P] [US1] Add usePinyinVisibility localStorage persistence test in src/hooks/usePinyinVisibility.test.ts - Test: Writes to localStorage on state change
- [ ] T003 [P] [US1] Add usePinyinVisibility restoration test in src/hooks/usePinyinVisibility.test.ts - Test: Restores from localStorage on subsequent runs
- [ ] T004 [P] [US1] Add usePinyinVisibility error handling test in src/hooks/usePinyinVisibility.test.ts - Test: Handles localStorage errors gracefully (returns default true)
- [ ] T005 [P] [US1] Create PinyinToggle component test file in src/components/PinyinToggle.test.tsx - Test: Renders Eye icon when visible=true
- [ ] T006 [P] [US1] Add PinyinToggle icon toggle test in src/components/PinyinToggle.test.tsx - Test: Renders EyeClosed icon when visible=false
- [ ] T007 [P] [US1] Add PinyinToggle click handler test in src/components/PinyinToggle.test.tsx - Test: Calls onToggle(!visible) on click
- [ ] T008 [P] [US1] Add PinyinToggle ARIA labels test in src/components/PinyinToggle.test.tsx - Test: Has correct ARIA labels ("Hide Pinyin" when visible, "Show Pinyin" when hidden)
- [ ] T009 [P] [US1] Add PinyinToggle ARIA pressed test in src/components/PinyinToggle.test.tsx - Test: Has aria-pressed matching visibility state
- [ ] T010 [P] [US1] Add RubyWord visibility test in src/components/RubyWord.test.tsx - Test: Always renders <ruby> and <rt> regardless of showPinyin prop
- [ ] T011 [P] [US1] Add RubyWord opacity class test (visible) in src/components/RubyWord.test.tsx - Test: When showPinyin=true, <rt> has opacity-100 class (or equivalent opacity: 1)
- [ ] T012 [P] [US1] Add RubyWord opacity class test (hidden) in src/components/RubyWord.test.tsx - Test: When showPinyin=false, <rt> has opacity-0 class (or equivalent opacity: 0)
- [ ] T012a [P] [US1] Add RubyWord transition test in src/components/RubyWord.test.tsx - Test: <rt> has transition-opacity duration-200 ease-in-out classes for smooth 200ms fade animation
- [ ] T013 [P] [US1] Add RubyWord Chinese characters test in src/components/RubyWord.test.tsx - Test: Always renders Chinese characters
- [ ] T014 [P] [US1] Add TextDisplay showPinyin prop test in src/components/TextDisplay.test.tsx - Test: Passes showPinyin prop to all RubyWord components
- [ ] T015 [P] [US1] Update App.test.tsx button count test in src/App.test.tsx - Test: Renders 4 title bar buttons (Pinyin, Theme, Fullscreen, Close)
- [ ] T016 [P] [US1] Create integration test file for pinyin toggle in tests/integration/pinyin-toggle.test.tsx - Test: End-to-end toggle flow (click → hide → click → show)
- [ ] T017 [P] [US1] Add persistence integration test in tests/integration/pinyin-toggle.test.tsx - Test: Preference persists after simulated page reload
- [ ] T018 [P] [US1] Add rapid toggle test in tests/integration/pinyin-toggle.test.tsx - Test: Multiple rapid toggles work correctly (no UI flicker)
- [ ] T019 [P] [US1] Create localStorage contract test file in tests/contract/localStorage.test.ts - Test: Writing "true" and reading returns "true"
- [ ] T020 [P] [US1] Add localStorage false value test in tests/contract/localStorage.test.ts - Test: Writing "false" and reading returns "false"
- [ ] T021 [P] [US1] Add localStorage missing key test in tests/contract/localStorage.test.ts - Test: Reading non-existent key returns null
- [ ] T022 [P] [US1] Add localStorage error handling test in tests/contract/localStorage.test.ts - Test: localStorage error handling (mock QuotaExceededError)

### Implementation for User Story 1

- [ ] T023 [P] [US1] Create usePinyinVisibility custom hook in src/hooks/usePinyinVisibility.ts following useTheme pattern - Lazy initialization, default to true, localStorage persistence with error handling
- [ ] T024 [P] [US1] Create PinyinToggle component in src/components/PinyinToggle.tsx - Clone ThemeToggle pattern, use Eye/EyeClosed icons from lucide-react, add onPointerDown stopPropagation, ARIA labels and aria-pressed
- [ ] T025 [US1] Modify RubyWord component in src/components/RubyWord.tsx - Add showPinyin prop, use CSS classes with opacity transition on <rt> element: className={`text-vermillion transition-opacity duration-200 ease-in-out ${showPinyin ? 'opacity-100' : 'opacity-0'}`} (CRITICAL: always render <rt>, never conditional rendering; 200ms transition matches existing hover pattern and complies with Constitution I)
- [ ] T026 [US1] Modify TextDisplay component in src/components/TextDisplay.tsx - Add showPinyin prop, pass to all RubyWord components
- [ ] T027 [US1] Modify App component in src/App.tsx - Import usePinyinVisibility, call hook, pass pinyinVisible to TextDisplay and PinyinToggle
- [ ] T028 [US1] Modify TitleBar component in src/components/TitleBar.tsx - Import PinyinToggle, add as first button in button group (order: Pinyin, Theme, Fullscreen, Close)
- [ ] T029 [US1] Run all tests for User Story 1 - Execute npm run test, verify all US1 tests pass

**Checkpoint**: At this point, User Story 1 should be fully functional and testable independently. Pinyin toggle works, preference persists, Chinese characters never move when toggling.
...
- [ ] T029a [P] [US2] Add ThemeToggle stopPropagation test in src/components/ThemeToggle.test.tsx - Test: Calls e.stopPropagation() on pointerDown event
- [ ] T029b [P] [US2] Add FullscreenToggle stopPropagation test in src/components/FullscreenToggle.test.tsx - Test: Calls e.stopPropagation() on pointerDown event
- [ ] T029c [P] [US2] Add CloseButton stopPropagation test in src/components/CloseButton.test.tsx - Test: Calls e.stopPropagation() on pointerDown event

### Implementation for User Story 2

- [ ] T030 [P] [US2] Modify ThemeToggle component in src/components/ThemeToggle.tsx - Add onPointerDown={(e) => e.stopPropagation()} to button element (prevents window dragging when clicking button)
- [ ] T031 [P] [US2] Modify FullscreenToggle component in src/components/FullscreenToggle.tsx - Add onPointerDown={(e) => e.stopPropagation()} to button element
- [ ] T032 [P] [US2] Modify CloseButton component in src/components/CloseButton.tsx - Add onPointerDown={(e) => e.stopPropagation()} to button element
- [ ] T033 [US2] Verify TitleBar component in src/components/TitleBar.tsx - Ensure data-tauri-drag-region attribute remains on <header> element, ensure <h1> title text has NO pointer-events: none or click handlers
...
- [ ] T034a [P] [US3] Add ThemeToggle sizing test in src/components/ThemeToggle.test.tsx - Test: Button has p-1.5 padding class and cursor-pointer class
- [ ] T034b [P] [US3] Add FullscreenToggle sizing test in src/components/FullscreenToggle.test.tsx - Test: Button has p-1.5 padding class and cursor-pointer class
- [ ] T034c [P] [US3] Add CloseButton sizing test in src/components/CloseButton.test.tsx - Test: Button has p-1.5 padding class and cursor-pointer class
- [ ] T034d [P] [US3] Add PinyinToggle sizing test in src/components/PinyinToggle.test.tsx - Test: Button has p-1.5 padding class and cursor-pointer class

### Implementation for User Story 3

- [ ] T035 [P] [US3] Modify ThemeToggle button sizing in src/components/ThemeToggle.tsx - Change padding from p-2 to p-1.5, add cursor-pointer class
- [ ] T036 [P] [US3] Modify FullscreenToggle button sizing in src/components/FullscreenToggle.tsx - Change padding from p-2 to p-1.5, add cursor-pointer class
- [ ] T037 [P] [US3] Modify CloseButton button sizing in src/components/CloseButton.tsx - Change padding from p-2 to p-1.5, add cursor-pointer class
- [ ] T038 [P] [US3] Modify PinyinToggle button sizing in src/components/PinyinToggle.tsx - Verify/ensure padding is p-1.5 (not p-2), add cursor-pointer class
- [ ] T039 [US3] Modify TitleBar button spacing in src/components/TitleBar.tsx - Change button container gap from gap-2 to gap-1
...
- [ ] T040a [US4] Create CSS cursor rules test in tests/unit/index.css.test.ts - Test: Verify src/index.css does NOT contain cursor: grab or cursor: grabbing rules for [data-tauri-drag-region] selector (use file read and regex/string match)

### Implementation for User Story 4

- [ ] T041 [US4] Modify global CSS in src/index.css - Delete lines 32-40 (drag region cursor rules: [data-tauri-drag-region] { cursor: grab; } and [data-tauri-drag-region]:active { cursor: grabbing; })
...
- [ ] T043 [P] Keyboard navigation test - Verify Tab key cycles through buttons in correct order (Pinyin → Theme → Fullscreen → Close), Enter key activates focused button (FR-017)
- [ ] T044 [P] Run full test suite - Execute npm run test and verify all tests pass (unit, integration, contract)
- [ ] T045 Build application - Execute npm run build and verify build succeeds with no errors
