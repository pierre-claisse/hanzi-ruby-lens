...
1. **Given** text is displayed at default size (100%), **When** user presses Ctrl-, **Then** text size decreases to 90% AND pinyin annotations scale proportionally AND title bar remains unchanged
2. **Given** text is at 100%, **When** user clicks the zoom-out button in title bar, **Then** text size decreases to 90% AND pinyin annotations scale proportionally AND title bar remains unchanged
3. **Given** text is at 100%, **When** user presses Ctrl- five times, **Then** text size decreases to 50%
4. **Given** text is at minimum zoom (50%), **When** user views title bar, **Then** zoom-out button appears visibly disabled
5. **Given** text is at minimum zoom (50%), **When** user presses Ctrl- or clicks zoom-out button, **Then** text remains at 50% (no further decrease)
...
3. **Given** zoom is at 70%, **When** user views the title bar, **Then** "(70%)" appears next to the title in subdued styling
...
2. **Given** user sets zoom to 70%, **When** app is closed and reopened, **Then** zoom level is restored to 70%
...
- What happens when user presses Ctrl- or clicks zoom-out button at 50% zoom? → No action, remains at 50%, AND zoom-out button appears visibly disabled
- What happens when zoom level is stored as intermediate value (e.g., 95%)? → INVALID STATE - zoom level must always be a multiple of 10 between 50% and 200%
...
- **FR-010**: Zoom level MUST be constrained between 50% minimum and 200% maximum
- **FR-011**: System MUST ignore zoom commands (keyboard or button) when at minimum (50%) or maximum (200%) boundaries
...
- **FR-023**: Zoom-out button MUST appear visibly disabled when zoom level is at 50% minimum
- **FR-024**: Zoom level MUST always be a multiple of 10 (50, 60, 70, ..., 190, 200) - intermediate values are invalid
...
- **Text Scale Level**: Percentage value (50-200%) representing current zoom level, default 100%, increments of 10%
- **Text Content Area**: The region displaying Chinese text and pinyin annotations (excludes title bar and controls)
- **Zoom Boundaries**: Minimum 50%, maximum 200%, enforced at both keyboard and button interactions
...
- **SC-002**: User can decrease text size from 100% to 50% in 5 keypresses (Ctrl- pressed 5 times) OR 5 button clicks
...
- **SC-006**: Text reflow handles zoom changes smoothly without layout breaks or overlapping elements at all zoom levels (50% to 200%)
...
- **SC-011**: Zoom-out button appears visibly disabled when zoom is at 50% minimum
- **SC-012**: Zoom level is always a multiple of 10 between 50% and 200% (no intermediate values like 95% or 105%)
...
- Visibly disabling zoom-in button at 200% maximum and zoom-out button at 50% minimum
...
- Enforcing zoom boundaries (50% minimum, 200% maximum)
- Ensuring zoom level is always a multiple of 10 (50, 60, 70, ..., 200)
...
- Minimum 50% zoom maintains sufficient readability for most content
...
- Zoom range MUST be constrained to 50%-200% to prevent unusable UI states
- Zoom level MUST always be a multiple of 10 between 50 and 200 (50, 60, 70, ..., 190, 200) - no intermediate values permitted
...
- Zoom-out button MUST appear visibly disabled at 50% minimum
