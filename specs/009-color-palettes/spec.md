...
1. **Given** the app is using the default palette ("Vermillion Scroll"), **When** user clicks the palette toggle button, **Then** a vertical dropdown appears listing all 7 available palettes
2. **Given** the palette dropdown is open, **When** user clicks "Jade Garden", **Then** the app immediately updates to the Jade Garden color scheme (background, text, and accent colors all change) AND the dropdown closes
3. **Given** the palette dropdown is open, **When** user presses Enter on a focused palette, **Then** the app immediately updates to that palette AND the dropdown closes
...
5. **Given** the app is in light mode using "Cinnabar & Smoke", **When** user toggles to dark mode, **Then** the dark mode colors use Cinnabar & Smoke's dark variant (charcoal black background, aged paper text, bright cinnabar accent)
...
5. **Given** the dropdown is open with "Plum Blossom" focused, **When** user presses Enter, **Then** "Plum Blossom" is selected as the active palette AND the dropdown closes AND focus returns to the palette toggle button
...
- What happens when user selects the palette that is already active? --> No visual change, dropdown closes normally
...
- **FR-001**: System MUST provide exactly 7 color palettes: Vermillion Scroll, Jade Garden, Indigo Silk, Cinnabar & Smoke, Plum Blossom, Golden Pavilion, Ink Wash
...
- **FR-006**: Palette dropdown MUST list all 7 palettes with their names
...
- **FR-009**: Currently focused palette MUST be visually highlighted in the dropdown, distinct from the selected indicator
...
- **FR-016**: Enter key MUST select the focused palette and close the dropdown
...
- **Color Palette**: A named set of three color tokens (background, text, accent) with light and dark mode variants. 7 palettes available: Vermillion Scroll, Jade Garden, Indigo Silk, Cinnabar & Smoke, Plum Blossom, Golden Pavilion, Ink Wash
...
- **SC-001**: User can switch between all 7 palettes and see colors change immediately (under 100ms) across the entire app
- **SC-002**: User can navigate the full palette dropdown using only the keyboard (Tab to reach, Enter to open, arrows to navigate, Enter to select, Tab or click outside to dismiss)
...
- **SC-005**: All 7 palettes render correctly in both light and dark themes (14 total color combinations produce readable text meeting WCAG AA contrast — minimum 4.5:1 for body text, 3:1 for large text). Accent-derived hover colors (e.g., `hover:bg-accent/24`) MUST remain visibly distinct against the background in all palettes
...
- **SC-008**: Pinyin annotations use the accent color of the selected palette across all 7 palettes in both light and dark modes
...
- Defining 7 named color palettes with light and dark mode variants (3 tokens each: background, text, accent)
...
- More than 7 palettes

## Assumptions

- The 7 palettes provide sufficient variety for the target audience (readers of Chinese text)
- All 7 palettes maintain readable contrast ratios in both light and dark modes
- The palette toggle dropdown does not need to scroll (7 items fit comfortably in the viewport)
...
- Palette selection MUST work independently from theme (light/dark) selection — all 14 combinations must be valid
...
The following 7 palettes are included. Each defines background, text, and accent (pinyin/highlights) tokens for both light and dark modes.
...
| Token     | Light                     | Dark                      |
|-----------|---------------------------|---------------------------|
| background | `#FEFCF3` (warm cream)    | `#1A1A2E` (deep navy)     |
| text      | `#2D2D2D` (charcoal)      | `#F5F0E8` (parchment)     |
| accent    | `#C84B31` (vermillion)    | `#C84B31` (vermillion)    |

### 2. Jade Garden
Celadon and bamboo. Cool greens on misty paper in light mode; a lantern-lit forest study in dark. Every accent catches the eye like sunlight through jade.

| Token     | Light                        | Dark                        |
|-----------|------------------------------|-----------------------------|
| background | `#F4F8F0` (morning mist)     | `#1B2420` (deep forest)     |
| text      | `#2A3A2E` (pine bark)        | `#E0EADB` (pale bamboo)     |
| accent    | `#2E8B57` (jade green)       | `#2E8B57` (jade green)      |

### 3. Indigo Silk
Blue-and-white porcelain meets dyed fabric. Raw silk crispness by day, midnight indigo depth by night. Moonlit blue accents float like brushstrokes on ceramic.

| Token     | Light                        | Dark                          |
|-----------|------------------------------|-------------------------------|
| background | `#F7F5F0` (raw silk)         | `#14162B` (midnight indigo)   |
| text      | `#2C2C3A` (deep slate)       | `#E8E4DD` (unbleached cotton) |
| accent    | `#3B5998` (porcelain blue)   | `#6B8CCE` (moonlit blue)      |

### 4. Cinnabar & Smoke
Maximum contrast, zero compromise. Fresh ink on rice paper, cinnabar red deeper than vermillion — the color of imperial seals. Dark mode burns text into inkstone black.

| Token     | Light                       | Dark                          |
|-----------|-----------------------------|-------------------------------|
| background | `#FAF7F2` (rice paper)      | `#1C1A17` (charcoal black)    |
| text      | `#1A1A1A` (fresh ink)       | `#F0EBE3` (aged paper)        |
| accent    | `#BF3030` (cinnabar red)    | `#D44040` (bright cinnabar)   |

### 5. Plum Blossom
Tang dynasty poetry in color form. Blush parchment and dark plum by day; twilight purple and evening pink by night. Warm, literary, gently romantic.

| Token     | Light                         | Dark                          |
|-----------|-------------------------------|-------------------------------|
| background | `#FBF5F3` (blush parchment)   | `#221C26` (twilight purple)   |
| text      | `#3A2D3D` (dark plum)         | `#F0E4E8` (plum mist)         |
| accent    | `#9B2D5E` (plum blossom)      | `#C75B8E` (evening plum)      |

### 6. Golden Pavilion
Forbidden City opulence distilled. Aged scrolls and imperial gold in light mode; lacquered wood and candlelit warmth in dark. Rich without being loud.

| Token     | Light                        | Dark                          |
|-----------|------------------------------|-------------------------------|
| background | `#FDF8EE` (golden cream)     | `#1F1B14` (lacquer brown)     |
| text      | `#352B1E` (dark walnut)      | `#EDE4D0` (warm ivory)        |
| accent    | `#C48820` (imperial gold)    | `#D4A030` (bright gold)       |
...
| Token     | Light                         | Dark                         |
|-----------|-------------------------------|------------------------------|
| background | `#F5F5F2` (cold rice paper)   | `#1D1D1D` (fresh ink)        |
